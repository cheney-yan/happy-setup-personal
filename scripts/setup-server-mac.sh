#!/bin/bash
# Happy Server Setup Script — macOS (launchd)
...
DAEMON_SCRIPT="$SERVER_DIR/scripts/daemon-start.sh"
PLIST_PATH="$HOME/Library/LaunchAgents/com.local.happy-server.plist"
...
command -v tailscale >/dev/null 2>&1 || die "Tailscale CLI required. Install from https://tailscale.com/download and enable MagicDNS."
...
cd "\$SERVER_DIR"
"\$NODE" "\$TSX" --env-file=.env.dev ./sources/standalone.ts migrate
exec "\$NODE" "\$TSX" --env-file=.env.dev ./sources/standalone.ts serve
SCRIPT
chmod +x "$DAEMON_SCRIPT"
mkdir -p "$SERVER_DIR/.logs"
log "daemon-start.sh written."

# ── 7. launchd daemon ────────────────────────────────────────────────────────
log "Step 7/9 — Installing daemon (launchd)..."
...
<dict>
    <key>Label</key>
    <string>com.local.happy-server</string>
    ...
    <string>$SERVER_DIR/.logs/launchd-stdout.log</string>
    <key>StandardErrorPath</key>
    <string>$SERVER_DIR/.logs/launchd-stderr.log</string>
</dict>
</plist>
PLIST
...
# ── 8. Tailscale HTTPS ────────────────────────────────────────────────────────
log "Step 8/9 — Tailscale HTTPS..."
echo ""
echo -e "${YELLOW}  WARNING: Always check existing Tailscale Serve config before proceeding.${NC}"
echo -e "${YELLOW}  Running 'tailscale serve --bg' OVERWRITES any existing config silently.${NC}"
echo ""
...
# Extract the HTTPS URL from tailscale
...
if echo "$AUTH_STATUS" | grep -q "Authenticated"; then
    ...
    echo "$AUTH_STATUS" | grep -q "Authenticated" || \
        die "Auth not complete. Re-run this script to retry from step 9."
fi
log "Auth verified ✓"
...
echo "  Server URL:  $HAPPY_SERVER_URL"
echo "  Logs:        $SERVER_DIR/.logs/"
echo "  Status:      launchctl list | grep happy-server"
echo "  Logs live:   tail -f $SERVER_DIR/.logs/launchd-stdout.log"
echo ""
echo "  To connect another machine, run on that machine:"
echo ""
echo -e "  ${YELLOW}HAPPY_SERVER_URL=$HAPPY_SERVER_URL \\"
echo -e "  bash <(curl -fsSL https://raw.githubusercontent.com/cheney-yan/happy-setup-personal/main/scripts/setup-machine-mac.sh)${NC}"
echo ""
